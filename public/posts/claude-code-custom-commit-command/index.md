...
差分の中に潜在的な問題（セキュリティ上の懸念、コードの品質など）があれば報告します。

### 3. コミットのグルーピング

以下の基準で変更をグループ化します：

- **機能単位**：1 コミット = 1 つの論理的な変更
- **ファイルタイプ別**：設定 → 型定義 → 実装 → テスト → ドキュメント
- **依存関係**：依存する変更は同じコミットにまとめる

### 4. コミットの実行

分析結果を表格形式で提示した後、**ユーザー確認なしで直接実行**します。
...
| ♻️    | refactor | リファクタリング                 |
| 🔥    | remove   | コードやファイルの削除           |
| 💄    | style    | UI やスタイルの追加・更新        |
| 🚸    | ux       | UX/ユーザビリティの改善          |
| ♿️   | a11y     | アクセシビリティの改善           |
| 📝    | docs     | ドキュメントの追加・更新         |
| 💡    | comment  | ソースコードのコメント追加・更新 |
| ✏️    | typo     | タイポ修正                       |
| 🤡    | mock     | モックの作成                     |
| 🏷️    | types    | 型定義の追加・更新               |
...
- [カスタムコマンドの作成方法](https://code.claude.com/docs/en/custom-commands)
